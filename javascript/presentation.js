...
function add_pub(title, guid){
	$('#pub_list').append('<input type="checkbox" name="pub[]" value="'+guid+'" checked=yes> '+title+'</input><br/>');
	$.facebox.close();
}

function tag(groupguid, pubguid, ts, token){
	$.ajax({
		url:wwwroot+'/action/presentation/tag',
...
function untag(groupguid, pubguid, ts, token){
	$.ajax({
                url:wwwroot+'action/presentation/untag',
                data:'groupguid='+groupguid+'&pubguid='+pubguid+'&__elgg_ts='+ts+'&__elgg_token='+token,
                success: function(){
