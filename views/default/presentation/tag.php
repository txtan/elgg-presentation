<?php
	...
	if(check_entity_relationship($groupguid, 'tagby', $pubguid)){
		$html .= <<< EOT
			<a id="$groupguid-$pubguid-tag" style='display:none' class='tagpubBtn' onclick="tag('$groupguid','$pubguid','$ts','$token')">tag as group presentation</a>
			<a id="$groupguid-$pubguid-untag" class='tagpubBtn' onclick="untag('$groupguid','$pubguid','$ts','$token')">untag</a>
EOT;
	}else{	
		$html .= <<< EOT
			<a id="$groupguid-$pubguid-tag" class='tagpubBtn' onclick="tag('$groupguid', '$pubguid', '$ts', '$token')">tag as group presentation</a>
